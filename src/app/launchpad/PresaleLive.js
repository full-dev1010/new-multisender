...
const PresaleLive = () => {
  ...
  const contributeToToken = () => {
    let addValue = 0;
    if( contributeAmount.trim() != '' ) addValue = parseFloat(contributeAmount);
    let checkMinBuy = parseFloat(infos.minBuy);
    let checkMaxBuy = parseFloat(infos.maxBuy);
    if(addValue < checkMinBuy){
      alert("Increase a bit");
      return;
    }
    if(addValue > checkMaxBuy){
      alert("Decrease a bit");
      return;
    }
    let presaleId = getPresaleIdFromLocation(window.location);
    contributeBNB(walletAddress, "" + presaleId, "" + addValue);
  }
  ...
  const asyncFetchTokenData = async () => {
    ...
    // console.log("CGI getRemainTime", presaleRemainTime.remainStartTime, presaleRemainTime.remainEndTime);
    
    setRemainStartTime(presaleRemainTime.remainStartTime);
    setRemainEndTime(presaleRemainTime.remainEndTime);
    // console.log("CGI getRemainTime State", remainStartTime, remainEndTime);
    
    if( presaleRemainTime.remainStartTime > 0 ){
      setInterval(setStartCountdownTime, 1000);
    }
    else if( presaleRemainTime.remainEndTime > 0 ){
      setInterval(setEndCountdownTime, 1000);
    }
  ...
  const setStartCountdownTime = () => {
    setRemainStartTime(remainStartTime => remainStartTime - 1000);
  }

  const setEndCountdownTime = () => {
    setRemainEndTime(remainEndTime => remainEndTime - 1000);
  }
          ...
          <div className="presale-card middle-card">
            <div className="middle-card-header">
              ...
              <div className="token-content">
                <p className="token-name">{infos.tokenSymbol}</p>
                <p className="token-style">{infos.tokenName}</p>
              </div>
            </div>
            ...
            <div className="contribute-button-container">
              <button className="btn btn-info btn-lg contribute"
                onClick={contributeToToken}
              >Contribute</button>
            </div>
            <div className="sale-end-time"><p>Sale ends in: {
              moment(new Date(parseInt(remainEndTime))).format('DD:hh:mm:ss')
            }</p></div>
