...
export default function LaunchpadListings() {
  ...
  const asyncFetchData = async () => {
    if( currentPresaleId >= presaleCount ) return;
    let updateData = contracts;

    const tokenInfo = await getTokenInfo(presaleCount - currentPresaleId - 1);
    const presaleInfo = await getPresaleInfo(presaleCount - currentPresaleId - 1);
    // const presaleAndTokenAddress = await getTokenAndPresaleAddress(presaleCount - currentPresaleId - 1);
    const weiRaised = await getPresaledTotalAmount(presaleCount - currentPresaleId - 1);
    const percent = calculatePercent(weiRaised, presaleInfo.hardCap);

    // TODO: should
    console.log("CGI getPresaleInfo(id)", presaleCount - currentPresaleId - 1);
    updateData.push(
      {
        presaleId: presaleCount - currentPresaleId - 1,
        tokenName: tokenInfo.name,
        tokenSymbol: tokenInfo.symbol,
        presaleStartTime: presaleInfo.presaleStartTime,
        presaleEndTime: presaleInfo.presaleEndTime,
    ...
    setCurrentPresaleId(currentPresaleId + 1)
  }
            ...
            <div className="col-xl-4 col-sm-6 grid-margin stretch-card" key={index}>
              <div className="card">
                <div className="card-body custom-card token-card social-card">
                  <div className="card-header-container">
                    ...
                    {contract.saleStatus === "active" &&
                      <div className="sales-time">
                        <p>Sales end in:</p>
                        <p>{
                          moment(new Date(parseInt(contract.presaleEndTime))).format('DD:hh:mm:ss') }</p>
                      </div>
                    }
                    {contract.saleStatus === "pending" &&
                      <div className="sales-time">
                        <p>Sales start in:</p>
                        <p>{
                          moment(new Date(parseInt(contract.presaleStartTime))).format('DD:hh:mm:ss') }</p>
                      </div>
                    }
                  ...
                  <div><p className="raised">Raised: {contract.weiRaised}/{contract.hardCap}</p></div>
                  <div><p className="under-raised">Soft Cap/Hard Cap: {contract.softCap}/{contract.hardCap} BNB</p></div>
                  <div><p className="double-under-raised">Min/Max contribution: {contract.minBuy}/{contract.maxBuy} BNB</p></div>
                  </Link>
